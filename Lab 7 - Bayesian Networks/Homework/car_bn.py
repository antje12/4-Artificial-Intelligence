...
class Variable(object):
        ...
        for row_key, row_val in self.probability_table.items():
            ...
            if valid_row:
                ...
                for mpi in marginal_parents_index:
                    parents_probability *= self.parents[mpi].get_marginal_probability(
                        row_key[mpi])
    ...
    def calculate_marginal_probability(self):
        ...
        if (len(self.parents) == 0):
            self.marginal_probabilities[0] = self.probability_table[()][0]
            self.marginal_probabilities[1] = self.probability_table[()][1]
        else:
            marginalF = 0 
            marginalT = 0

            # rows of the probability table
            # key is based on parents probabilities
            for key, value in self.probability_table.items():                

                # probability of row happening given parents
                f = value[0]
                t = value[1]

                # the key part for the i'th parent
                for i, pKey in enumerate(key):
                    parent = self.parents[i]
                    # get probability of parent happening
                    if (pKey == "false"):
                        f *= parent.marginal_probabilities[0]
                        t *= parent.marginal_probabilities[0]
                    else:
                        f *= parent.marginal_probabilities[1]
                        t *= parent.marginal_probabilities[1]

                # add to marginal probability (probability of event occurring, independent of other events)
                marginalF += f
                marginalT += t

            self.marginal_probabilities[0] = marginalF
            self.marginal_probabilities[1] = marginalT
...
class BayesianNetwork(object):
    ...
    def get_joint_probability(self, values):
        ...
        for var in reversed(self.variables):
            ...
            key = ()
            for parent in reversed(var.parents):
                parentValue = values[parent.name]
                key = key + (parentValue,)

            row = var.probability_table[key]
    ...
    def get_conditional_probability(self, values, evidents):
        ...
        if all(self.varsMap[list(values.keys())[0]].is_child_of(self.varsMap[evident]) for evident in evidents.keys()):
            ...
            for child, c_val in values.items():
                res *= self.varsMap[child].get_conditional_probability(
                    c_val, evidents)
        ...
        # assumption: nodes in each level are independent, given their parents
            ...
            for parent, p_val in values.items():
                joint_marginal_parents *= self.varsMap[parent].get_marginal_probability(
                    p_val)
            ...
            for child, c_val in evidents.items():
                joint_marginal_children *= self.varsMap[child].get_marginal_probability(
                    c_val)

                # children given their parents. here the values become the
                # evidents!
                joint_conditional_children *= self.varsMap[child].get_conditional_probability(
                    c_val, values)
                ...
                marginal_of_evidents = marginal_of_evidents * \
                    self.varsMap[child].get_conditional_probability(
                        c_val, complementary_conditional_values)

                # print("Child: {}".format(child))
                # print("    Given: {}".format(complementary_conditional_values))

            # uses Bayes rule, for calculating the conditional probability
            res = (joint_conditional_children * joint_marginal_parents) / ((joint_conditional_children *
                                                                            joint_marginal_parents) + marginal_of_evidents * (1 - joint_marginal_parents))
...
def car():
    ...
    DT = Variable('DamagedTire', ('false', 'true'), t1)
    FTL = Variable('FuelTankLeaking', ('false', 'true'), t2)
    EM = Variable('ElectronicsMalfunctioning', ('false', 'true'), t3)
    
    SMS = Variable('SlowMaxSpeed', ('false', 'true'), t4, [DT, EM])
    HC = Variable('HighConsumption', ('false', 'true'), t5, [DT, FTL, EM])
    ...
    # Marginal = probability of event occurring, independent of other events
    ...
    joint_values = {
        'DamagedTire': 'true',
        'FuelTankLeaking': 'true',
        'ElectronicsMalfunctioning': 'true',
        'SlowMaxSpeed': 'true',
    ...
    # test for Electronics Malfunctioning
