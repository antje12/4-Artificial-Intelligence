...
def mutate(individual):
    ...
    mutation[choosen_bit] = random.randint(0, 1)
...
def random_selection(population, fitness_fn):
    ...
    while father == None:
        for individual in population:
            fitness = fitness_fn(individual)
            percentage_contribution = fitness/total_fitness
            dice = random.uniform(0, 1)
            if dice < percentage_contribution:
                if mother == None:
                    mother = individual
                else:
                    father = individual
...
def fitness_function(individual):
    ...
    '''
    fitness = 0
    for element in enumerate(reversed(individual)):
        index = element[0]
        value = element[1]
        if index == 0:
            fitness += 1*1*value
        else:
            fitness += index*2*value
